...
def get_accelerate_model(args, checkpoint_dir):
    ...
    max_memory = f'{args.max_memory_MB}MB'  # 单块GPU的最大显存
    ...
    if not args.full_finetune:
        if checkpoint_dir is not None:
            ...
            model = PeftModel.from_pretrained(model, join(checkpoint_dir, 'default'), is_trainable=True)
        else:
...
def local_dataset(dataset_name):
    ...
    加载数据集
    """
    print('dataset_name:', dataset_name)  # ./data/test_data/alpaca_data.json
    ...
    # print('type(full_dataset):', type(full_dataset))  # <class 'datasets.arrow_dataset.Dataset'>
    split_dataset = full_dataset.train_test_split(test_size=0.1)  # 训练测试集分离
...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
    ...
    def load_data(dataset_name):
        ...
        if dataset_name == 'alpaca':
            ...
            print("Load a custom dataset")
            """加载自定义数据集"""
            if os.path.exists(dataset_name):
                # print('dataset_name:', dataset_name)
                try:
                    args.dataset_format = args.dataset_format if args.dataset_format else "input-output"  # 数据集格式
                    full_dataset = local_dataset(dataset_name)  # 训练集&测试集
                    return full_dataset
                except:
    ...
    def format_dataset(dataset, dataset_format):
        ...
        """
        test.jsonl dataset --> 695 in total
        DatasetDict({
            train: Dataset({
        ...
        )  # TODO: 可能需要保存instruction(进行instruction tuning时)
        return dataset

    # Load dataset. 数据加载&格式化
    ...
    """默认情况下do_train=True，如果需要进行evaluate或者predict需要在命令行手动增加参数项"""
    ...
    if args.do_train:
        ...
        """将序列分成相同长度的批次，节省memory并大大加快训练速度"""
        if args.group_by_length:
    ...
    """数据收集器:让数据适配模型，可能进行截断、填充、标记化等操作"""
...
def train():
    ...
    'data_collator': DataCollatorForCausalLM(tokenizer=LlamaTokenizer(name_or_path='./llama2-13b-orca-8k-3319', 
        vocab_size=32000, model_max_length=1000000000000000019884624838656, is_fast=False, 
        padding_side='right', truncation_side='right', special_tokens={'bos_token': '<s>', 
        'eos_token': '</s>', 'unk_token': '<unk>', 'sep_token': '<s>', 'pad_token': '</s>',
        'additional_special_tokens': ['<|assistant|>', '<|prefix_end|>', '<|system|>', '<|prefix_begin|>',
         '<|prompter|>']}, clean_up_tokenization_spaces=False), source_max_len=1024, target_max_len=256,
    ...
    # 在训练前验证数据类型和参数统计
    print('Trainable parameter type & num')
    """
    trainable params: 250347520 || all params: 13266375680 || trainable: 1.8870829986928277
    torch.float32 578606080 0.04361448024363501
    torch.int8 12687769600 0.956385519756365
    """
