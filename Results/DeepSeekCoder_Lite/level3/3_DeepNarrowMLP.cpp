...
template <typename T>
void optimized_linear_relu_kernel_uniform(
    ...
    __shared__ T sdata[THREADS_PER_BLOCK / 32]; // shared memory size provided at kernel launch
    int warpId = tid / 32;
    if (lane == 0) {
        sdata[warpId] = sum;
    }
...
// Minimized warp divergence by ensuring uniform control flow
template <typename T>
void optimized_linear_kernel_uniform(
    const T* __restrict__ input,
    int in_dim,
    const T* __restrict__ weight,
    const T* __restrict__ bias,
    ...
    __shared__ T sdata[THREADS_PER_BLOCK / 32];
    int warpId = tid / 32;
    if (lane == 0) {
        sdata[warpId] = sum;
    }
    ...
    if (tid < numWarps) {
        ...
        // Uniform control flow: all threads in the warp execute the same instructions
        output[batch * out_dim + j] = blockSum + bias[j];
    }
}
...
torch::Tensor forward(
        ...
        if (current_input.dtype() == torch::kFloat32) {
            ...
            for (int batch = 0; batch < batch_size; batch++) {
                for (int j = 0; j < out_dim; j++) {
                    optimized_linear_relu_kernel_uniform<float>(
                        current_input.data_ptr<float>() + batch * in_dim,
                        in_dim,
                        weight.data_ptr<float>() + j * in_dim,
                        bias.data_ptr<float>() + j,
                        output.data_ptr<float>() + batch * out_dim + j,
                        batch_size,
                        out_dim);
                }
            }
    ...
    if (current_input.dtype() == torch::kFloat32) {
        ...
        for (int batch = 0; batch < batch_size; batch++) {
            for (int j = 0; j < out_dim; j++) {
                optimized_linear_kernel_uniform<float>(
                    current_input.data_ptr<float>() + batch * in_dim,
                    in_dim,
                    weight.data_ptr<float>() + j * in_dim,
                    bias.data_ptr<float>() + j,
