...
torch::Tensor fire_forward(
    ...
    // Ensure input tensors are contiguous and on the CPU
    x = x.contiguous();
    squeeze_weight = squeeze_weight.contiguous();
    squeeze_bias = squeeze_bias.contiguous();
    expand1x1_weight = expand1x1_weight.contiguous();
    expand1x1_bias = expand1x1_bias.contiguous();
    expand3x3_weight = expand3x3_weight.contiguous();
    expand3x3_bias = expand3x3_bias.contiguous();
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &fire_forward, "SqueezeNet Fire Module forward (CPU)");
}