...
at::Tensor forward(
    ...
    // Perform the kernel in parallel
    #pragma omp parallel for
    for (int64_t b = 0; b < B; ++b) {
        for (int64_t h = 0; h < n_head; ++h) {
            for (int64_t i = 0; i < T; ++i) {
                for (int64_t j = 0; j < T; ++j) {
                    float val = att[b][i][j] * scale;
                    bool is_masked = (bias_data[i * T + j] == 0.0f);
                    val = is_masked ? -std::numeric_limits<float>::infinity() : val;
                    att[b][i][j] = std::fmaxf(val, 0.0f);
                }
            }
        }
    }
