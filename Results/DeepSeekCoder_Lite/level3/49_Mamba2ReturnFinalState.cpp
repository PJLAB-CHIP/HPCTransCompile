...
// It evenly distributes the workload across threads and blocks.
// Each thread computes one element of the final output by iterating over the T dimension.

void balanced_final_state_kernel(
    ...
    int tile_size = FINAL_TILE_SIZE;

    #pragma omp parallel for schedule(dynamic, 1)
    for (int global_output = 0; global_output < total_outputs; ++global_output) {
        int p_idx = global_output / n_size;
        int n_idx = global_output % n_size;

        // Compute cumulative sum and weights
        std::vector<float> s_data(T, 0.0f);
        std::vector<float> w_data(T, 0.0f);
        for (int tid = 0; tid < T; ++tid) {
            int idx = (p_idx * n_size + n_idx) * T + tid;
            s_data[tid] = A_tail_pad[idx];
        }

        // Parallel prefix sum to compute cumulative sum
        ...
        // Compute weights
        for (int tid = 0; tid < T; ++tid) {
            w_data[tid] = expf(s_data[T - 1] - s_data[tid]);
        }

        // Compute the dot product for the output element
        float sum_val = 0.0f;
        for (int t = 0; t < T; ++t) {
            int state_idx = (((p_idx * n_size + n_idx) * T + t) * h_size + h_idx) * p_size + p_idx;
            sum_val += w_data[t] * states_cat[state_idx];
        }

        // Write the result to final_state
        int out_idx = (((p_idx * n_size + n_idx) * h_size + h_idx) * p_size + p_idx) * n_size + n_idx;
