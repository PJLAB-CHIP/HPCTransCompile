    ...
    for (int idx = 0; idx < total; idx++) {
        ...
        for (int i = 0; i < pool_h; i++) {
            for (int j = 0; j < pool_w; j++) {
                ...
                // Apply ReLU inline
                float relu_val = std::fmaxf(val, 0.0f);
                if (relu_val > max_val) {
                    max_val = relu_val;
                }
            }
...
void flatten_kernel(const float* __restrict__ input, float* __restrict__ output, int total) {
    #pragma omp parallel for
    for (int idx = 0; idx < total; idx++) {
