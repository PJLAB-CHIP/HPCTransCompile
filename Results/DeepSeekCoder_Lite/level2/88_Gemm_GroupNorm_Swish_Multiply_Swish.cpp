...
torch::Tensor module_fn_cpu_forward(
    ...
    const int chunk_size = (N + omp_get_max_threads() - 1) / omp_get_max_threads();
    ...
    for(int i = 0; i < omp_get_max_threads(); i++) {
        ...
        auto stream = at::cuda::getDefaultCUDAStream();  // Use default stream for CPU

        dim3 blocks(ceil((end_n - start_n) / (float)chunk_size), num_groups);
        int threads = std::min(channels_per_group, 1024);

        AT_DISPATCH_FLOATING_TYPES(x_linear.scalar_type(), "module_fn_cpu_forward", ([&] {
