...
// It caches the bias values in shared memory to reduce redundant global memory accesses.
// The kernel then processes the output tensor in a vectorized manner using float4 loads/stores.

void shared_mem_post_process_kernel(
    ...
    for (int i = threadIdx.x; i < channels; i += num_threads) {
    ...
    // Process vectorized elements in groups of 4
    ...
    for (int i = tid; i < remainder; i += num_threads) {
...
torch::Tensor forward(
    ...
    #pragma omp parallel
    {
        ...
        shared_mem_post_process_kernel(
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &forward, "Shared-memory bias optimized post-processing kernel with minimal synchronization (CPU)");
}