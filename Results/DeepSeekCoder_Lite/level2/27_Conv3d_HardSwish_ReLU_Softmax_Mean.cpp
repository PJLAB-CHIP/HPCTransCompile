...
void ldg_aligned_fused_kernel(const float* __restrict__ input,
...
torch::Tensor module_forward(
    ...
    conv_weight = conv_weight.contiguous().to(torch::kCUDA);
    conv_bias = conv_bias.contiguous().to(torch::kCUDA);
    ...
    for (int batch_idx = 0; batch_idx < batch_size; ++batch_idx) {
        for (int spatial_idx = 0; spatial_idx < spatial_size; ++spatial_idx) {
            ldg_aligned_fused_kernel(x.data_ptr<float>(),
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &module_forward, "Fused CPU module forward with __ldg() and aligned global memory accesses");
}