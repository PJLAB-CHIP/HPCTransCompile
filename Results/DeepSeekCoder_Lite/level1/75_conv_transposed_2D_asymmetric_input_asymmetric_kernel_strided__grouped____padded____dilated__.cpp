...
// 2D Tiled CPU kernel for 2D transposed convolution
// Each thread computes a tile of outputs for a given batch index.
...
// Host function wrapping the CPU kernel
at::Tensor forward(
    at::Tensor x,
    at::Tensor weight,
  ...
  // Parallelize over the grid
  #pragma omp parallel for
  for (int i = 0; i < batch; ++i) {
    conv_transpose2d_kernel_tiled<<<grid, block>>>(
        x.data_ptr<float>() + i * (in_channels * in_h * in_w),
        weight.data_ptr<float>(),
        bias.value().data_ptr<float>(),
        output.data_ptr<float>() + i * (out_channels * out_h * out_w),
        batch,
        in_channels,
        in_h,
        in_w,
