...
torch::Tensor unroll_tuned_log_softmax_cuda_forward(torch::Tensor input, int64_t dim) {
    ...
    AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "unroll_tuned_log_softmax_forward_cuda", ([&] {
        ...
        for (int64_t batch_idx = 0; batch_idx < batch_size; ++batch_idx) {
            const scalar_t* input_row = input_data + batch_idx * dim_size;
            scalar_t* output_row = output_data + batch_idx * dim_size;

            // Shared memory for reductions
            std::vector<scalar_t> sdata(optimal_block_size);

            // Phase 1: Compute the maximum value in the row
            scalar_t local_max = -std::numeric_limits<scalar_t>::infinity();
            for (int64_t idx = threadIdx.x; idx < dim_size; idx += optimal_block_size) {
            ...
            for (int stride = optimal_block_size / 2; stride > 0; stride >>= 1) {
