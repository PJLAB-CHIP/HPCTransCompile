        ...
        for (int oc = 0; oc < OC; ++oc) {
            for (int h = 0; h < H; ++h) {
                for (int w = 0; w < W; ++w) {
                    ...
                    int index = b * OC * H * W + oc * H * W + h * W + w;
...
torch::Tensor forward_cpu(
    ...
    TORCH_CHECK(weight.device().is_cpu(), "weight must be CPU tensor");
    if (bias.has_value()) {
        TORCH_CHECK(bias.value().device().is_cpu(), "bias must be CPU tensor");
    }
    ...
    const float* b_ptr = bias ? bias.value().data_ptr<float>() : nullptr;
    float* out_ptr = output.data_ptr<float>();

    forward_kernel(x_ptr, w_ptr, b_ptr, out_ptr, B, IC, OC, H, W);

    return output;
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &forward_cpu, "Pointwise 2D convolution forward (CPU)");
}