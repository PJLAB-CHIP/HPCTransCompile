...
void min_reduce_warp_kernel(
  ...
  int warpId = (omp_get_thread_num() / 32);
...
torch::Tensor forward(torch::Tensor input, int64_t dim) {
  ...
  AT_DISPATCH_ALL_TYPES(input.scalar_type(), "min_reduce_warp_cuda", ([&] {
    ...
    for (int warpId = 0; warpId < total_warps; ++warpId) {
      min_reduce_warp_kernel<scalar_t>(
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("forward", &forward, "Min reduction over a specified dimension using warp-level primitives (CPU)");
}