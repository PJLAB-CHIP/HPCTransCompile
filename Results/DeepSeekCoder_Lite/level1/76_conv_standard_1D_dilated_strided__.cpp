            ...
            for (int o = 0; o < out_size; ++o) {
                ...
                if (end_pos < in_size) {
                    for (int ic = 0; ic < in_channels; ++ic) {
                        const float* x_base = x_data + b * (in_channels * in_size) + ic * in_size + start_pos;
                        const float* w_base = weight_data + oc * (in_channels * kernel_size) + ic * kernel_size;
                    ...
                    for (int ic = 0; ic < in_channels; ++ic) {
                        const float* x_base = x_data + b * (in_channels * in_size) + ic * in_size;
                        const float* w_base = weight_data + oc * (in_channels * kernel_size) + ic * kernel_size;
                ...
                output_data[b * (out_channels * out_size) + oc * out_size + o] = sum;
            }
        }
    }
