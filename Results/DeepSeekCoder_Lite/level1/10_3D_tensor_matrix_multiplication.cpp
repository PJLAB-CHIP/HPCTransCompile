...
// To minimize loop overhead, critical loops are unrolled with #pragma unroll. The function uses shared memory
// tiling with coalesced global memory loads (with __ldg) and branchless boundary handling when possible.

template <typename scalar_t>
void unrolled_tiled_kernel(
...
torch::Tensor module_fn_forward(
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(A.scalar_type(), "unrolled_tiled_kernel", ([&] {
        unrolled_tiled_kernel<scalar_t>(
            A.data_ptr<scalar_t>(),
            B.data_ptr<scalar_t>(),
