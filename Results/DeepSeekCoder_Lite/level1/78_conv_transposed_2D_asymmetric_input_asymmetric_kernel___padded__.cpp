...
void conv_transpose2d_forward_kernel_cpu(
    ...
    const int ty = 0; // Assuming single thread in y-direction for simplicity
    const int bx = omp_get_thread_num(); // Assuming single block in x-direction for simplicity
    const int by = 0; // Assuming single block in y-direction for simplicity
    const int bz = omp_get_thread_num(); // Assuming single block in z-direction for simplicity
...
void get_optimal_block_config_cpu(
    int H_out, 
    int W_out, 
    int& block_dim_x, 
    int& block_dim_y,
...
torch::Tensor conv_transpose2d_forward_cpu(
    ...
    get_optimal_block_config_cpu(H_out, W_out, block_dim_x, block_dim_y, N, C_out);

    // Launch kernel with dynamic block size selection
    conv_transpose2d_forward_kernel_cpu<8, 8>(
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &conv_transpose2d_forward_cpu, "Conv Transpose 2D forward with optimized block sizes",
          py::arg("x"),
          py::arg("weight"),
          py::arg("bias") = py::none(),
          py::arg("stride"),
