...
inline void warp_scan(
    ...
    for (int offset = 1; offset < WARP_SIZE; offset <<= 1) {
        scalar_t y = __shfl_up(val, offset);
...
template <typename scalar_t>
void modular_hybrid_masked_cumsum_kernel(
    ...
    const int row = omp_get_thread_num();
    if (row >= N) return;
    ...
    if (L <= PARALLEL_THRESHOLD) {
        process_small_row(x_row, mask_row, out_row, omp_get_thread_num(), L);
    } else {
        __shared__ scalar_t row_offset;
        if (omp_get_thread_num() == 0) row_offset = 0;
        __sync();
...
torch::Tensor masked_cumsum(
    ...
    AT_DISPATCH_FLOATING_TYPES(x.scalar_type(), "modular_hybrid_masked_cumsum", ([&] {
        modular_hybrid_masked_cumsum_kernel<scalar_t>(
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &masked_cumsum, "Modular Hybrid Masked Cumulative Sum (CPU)");
}