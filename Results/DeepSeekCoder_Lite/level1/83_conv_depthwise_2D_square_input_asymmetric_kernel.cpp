...
torch::Tensor forward(
    ...
    torch::Tensor bias_val = bias.has_value() && bias.value().defined() 
        ? bias.value().contiguous() 
        : torch::zeros({channels}, x.options());
    
    const int out_h = (in_h + 2 * padding - dilation * (kernel_h - 1) - 1) / stride + 1;
    const int out_w = (in_w + 2 * padding - 1) / stride + 1;
    
    torch::Tensor output = torch::empty({batch, channels, out_h, out_w}, x.options());
    
    #pragma omp parallel for
    for (int bc_idx = 0; bc_idx < batch * channels; ++bc_idx) {
        const int b = bc_idx / channels;
        const int c = bc_idx % channels;
        
        const float bias_val_c = bias_val[c];
            ...
            for (int ow = 0; ow < out_w; ++ow) {
                ...
                for (int kh = 0; kh < kernel_h; ++kh) {
                    ...
                    if (ih >= 0 && ih < in_h && iw >= 0 && iw < in_w) {
                        int input_idx = ((b * channels + c) * in_h + ih) * in_w + iw;
                        int weight_idx = c * kernel_h + kh;
                        sum += x[b][c][ih][iw].item<float>() * weight[c][kh].item<float>();
                    }
                }
                sum += bias_val_c;
