...
// This function computes argmax over a specified dimension using only warp-level primitives.
// Each thread processes several elements along the reduction dimension in a stride loop,
// utilizing shared memory for improved performance.
...
torch::Tensor argmax_forward_cpu(const torch::Tensor& x, const int64_t dim) {
    ...
    for (int idx = 0; idx < total; idx++) {
        ...
        for (int d = 0; d < dimSize; d += warpSize) {
            // Use __ldg to enable read-only cache and improved performance
            float val = x_contig.data_ptr<float>()[start_offset + d * innerSize];
            if (val > thread_max) {
                thread_max = val;
                thread_arg = d;
        ...
        indices.data_ptr<int64_t>()[idx] = thread_arg;
    }
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &argmax_forward_cpu, "ArgMax CPU forward (warp-level reduction, no shared memory)");
}