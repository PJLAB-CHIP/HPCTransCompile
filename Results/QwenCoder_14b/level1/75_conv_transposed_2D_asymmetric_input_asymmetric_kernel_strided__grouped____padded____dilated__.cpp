...
// Inline function to compute greatest common divisor
inline int gcd_device(int a, int b) {
  while(b != 0) {
    int t = b;
    b = a % b;
    a = t;
...
// Inline function for minimum
inline int min_device(int a, int b) {
  return (a < b) ? a : b;
}

// 2D Tiled CPU function for 2D transposed convolution
void conv_transpose2d_cpu_tiled(
    const float* x,
    const float* weight,
    const float* bias,
    float* output,
  ...
  for (int n = 0; n < batch; ++n) {
    for (int oc = 0; oc < out_channels; ++oc) {
      for (int sp_index = 0; sp_index < out_h * out_w; ++sp_index) {
...
// Host function wrapping the CPU function
at::Tensor forward(
    at::Tensor x,
    at::Tensor weight,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("forward", &forward, "2D Transposed Convolution with Tiling and Shared Bias (CPU)",
        py::arg("x"),
        py::arg("weight"),
        py::arg("bias") = py::none(),
        py::arg("stride"),
