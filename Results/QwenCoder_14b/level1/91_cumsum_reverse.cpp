...
at::Tensor reverse_cumsum_cpu(at::Tensor x, int64_t dim) {
    // Ensure the tensor is contiguous and on CPU
    x = x.contiguous();
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &reverse_cumsum_cpu, "Reverse cumulative sum along a specified dimension (CPU)");
}