...
inline scalar_t apply_mask(scalar_t val, bool mask) {
...
void process_small_row(
    ...
    scalar_t warp_sums[WARP_SIZE] = {0};  // Support up to 32 warps
...
void process_tile(
    ...
    const int count = std::min(TILE_SIZE, L - tile_start);
...
void modular_hybrid_masked_cumsum_cpu(
    ...
    for (int64_t row = 0; row < N; ++row) {
        const scalar_t* x_row = x_flat.data_ptr<scalar_t>() + row * L;
        const bool* mask_row = mask_flat.data_ptr<bool>() + row * L;
        scalar_t* out_row = output_flat.data_ptr<scalar_t>() + row * L;
        
        if (L <= PARALLEL_THRESHOLD) {
            process_small_row(x_row, mask_row, out_row, tid, L);
        } else {
            scalar_t row_offset = 0;
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &modular_hybrid_masked_cumsum_cpu, "Modular Hybrid Masked Cumulative Sum (CPU)");
}
