...
torch::Tensor forward(
    ...
    x = x.to(torch::kCPU);
    branch1x1_weight = branch1x1_weight.to(torch::kCPU);
    branch1x1_bias = branch1x1_bias.to(torch::kCPU);
    branch3x3_reduce_weight = branch3x3_reduce_weight.to(torch::kCPU);
    branch3x3_reduce_bias = branch3x3_reduce_bias.to(torch::kCPU);
    branch3x3_weight = branch3x3_weight.to(torch::kCPU);
    branch3x3_bias = branch3x3_bias.to(torch::kCPU);
    branch5x5_reduce_weight = branch5x5_reduce_weight.to(torch::kCPU);
    branch5x5_reduce_bias = branch5x5_reduce_bias.to(torch::kCPU);
    branch5x5_weight = branch5x5_weight.to(torch::kCPU);
    branch5x5_bias = branch5x5_bias.to(torch::kCPU);
    branch_pool_conv_weight = branch_pool_conv_weight.to(torch::kCPU);
    branch_pool_conv_bias = branch_pool_conv_bias.to(torch::kCPU);
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &forward, "Inception module forward (CPU)");
}