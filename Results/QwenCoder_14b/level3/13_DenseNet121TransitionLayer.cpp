...
// CPU function for ReLU activation
template <typename scalar_t>
void relu_cpu(const scalar_t* input, scalar_t* output, int64_t size) {
...
// Wrapper function for ReLU CPU
torch::Tensor relu_cpu(torch::Tensor input) {
    auto output = torch::empty_like(input);
    auto size = input.numel();
    relu_cpu<scalar_t>(input.data_ptr<scalar_t>(), output.data_ptr<scalar_t>(), size);
    return output;
}
...
torch::Tensor module_forward(
    ...
    // ReLU activation using custom CPU function
    x = relu_cpu(x);
