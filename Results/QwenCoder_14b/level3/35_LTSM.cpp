...
void lstm_elementwise_stride_cpu(
    const float* gates,
    const float* prev_c,
    float* h,
    float* c,
...
torch::Tensor lstm_forward_stride_cpu(
    torch::Tensor input,
    torch::Tensor w_ih,
    torch::Tensor w_hh,
    torch::Tensor b_ih,
...
torch::Tensor linear_forward_stride_cpu(
    torch::Tensor input,
    torch::Tensor weight,
    torch::Tensor bias
) {
...
torch::Tensor forward_cpu(
    ...
    return linear_forward_stride_cpu(out.select(1, -1), fc_weight, fc_bias);
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &forward_cpu, "CPU optimized LSTM");
}