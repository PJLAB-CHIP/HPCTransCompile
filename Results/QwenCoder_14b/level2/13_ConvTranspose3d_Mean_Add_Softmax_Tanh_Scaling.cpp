...
void fused_operations_cpu(
    const float* input,
    const float* conv_weight,
    const float* conv_bias,
    const float* spatial_bias,
        ...
        for (int d = 0; d < out_depth; ++d) {
            for (int h = 0; h < out_height; ++h) {
                for (int w = 0; w < out_width; ++w) {
                    ...
                    output[b * out_depth * out_height * out_width + d * out_height * out_width + h * out_width + w] = tanhf(biased) * scaling_factor;
                }
            }
        }
    }
