...
#### Utils ####
from utils.api_utils import local_call
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generated target platform code."
    )
...
def no_pass_generate_batch(model_name, prompt_generator: PromptGenerator):
    no_pass_dict = {
        ...
        "level2": [
            "79_Conv3d_Multiply_InstanceNorm_Clamp_Multiply_Max",
...
def generate(
    ...
    code = prompt_generator.load_source_code_single(
        level, operator, action=action, return_code=True, model_name=model_name
    )
    base_prompt, system_prompt = prompt_generator.fill_template(
        code, action=action, operator=operator
    )
    content = local_call(
        model_name, system_prompt, base_prompt, device, use_lora=use_lora
    )

    if action == "optimization_c":
        if use_lora:
            save_path = join(
                prompt_generator.save_path_dict[action],
                f"{model_name}_optim_lora",
                level,
            )
        else:
            save_path = join(
        ...
        if use_lora:
            ...
            save_path = join(
                prompt_generator.save_path_dict[action], model_name, level
            )
    ...
    save_to_file(
        content,
        operator,
        suffix=prompt_generator.suffix_dict[action],
        action=action,
        save_path=save_path,
    )
...
def generate_batch(
    ...
    if range == "left":
        operators = extract_residual_operator(
            model_name, level, use_lora, action
        )
    elif range == "all":
        if action == "translation_c_tvm":
            # base_path = "/code/LLM4HPCTransCompile/LLM4HPC/AI_CUDA_Engineer/CUDA_Code"
            base_path = "/code/LLM4HPCTransCompile/HPCTransEval/benchmark/data"
        else:
            base_path = "/code/LLM4HPCTransCompile/ParallelBench"

        operators = [
            os.path.splitext(file)[0]
            for file in os.listdir(join(base_path, level))
            if os.path.isfile(join(base_path, level, file))
            and file.endswith(".cu")
        ]
    print("====Operators====")
    print(operators)
    for operator in tqdm(operators):
        generate(
            model_name, prompt_generator, level, operator, action, use_lora
        )


def extract_residual_operator(model_name, level, use_lora, action):
    all_operators_path = Path("/code/KernelBench/KernelBench", level)
    all_operators = [
        os.path.splitext(file)[0]
        for file in os.listdir(all_operators_path)
        if os.path.isfile(join(all_operators_path, file))
    ]
    base_path = f"/code/LLM4HPCTransCompile/Results/{model_name}"
    if use_lora:
        base_path += "_lora"
    if action == "translation_c_tvm":
        base_path += "_tvm"
    generated_operators_path = Path(base_path) / f"{level}"
    generated_operators_path.mkdir(parents=True, exist_ok=True)
    generated_operators = [
        os.path.splitext(file)[0]
        for file in os.listdir(generated_operators_path)
        if os.path.isfile(join(generated_operators_path, file))
    ]
...
def extract_optim_operator(model_name, level):
    ...
    with open(
        join(
            "/code/LLM4HPCTransCompile/Results",
            args.model_name,
            f"{args.level[-1]}_eval_results.json",
        ),
        "r",
        encoding="utf-8",
    ) as file:
